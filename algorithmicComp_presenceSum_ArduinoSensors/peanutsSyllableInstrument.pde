...
class PeanutsSyllableInstrument implements Instrument
{
    ...
    Wavetable baseWave, AudioOutput output )
  {
    ...
    float lpFixedFreq = lpFixedLoFreq + (float)Math.random()*( lpFixedHiFreq - lpFixedLoFreq )*5;
    float bp1Freq = bp1LoFreq + /*arduino.AnalogRead(*)*/(float)Math.random()*( bp1HiFreq - bp1LoFreq );
    float bp2Freq = bp2LoFreq + /*arduino.AnalogRead(*)*/(float)Math.random()*( bp2HiFreq - bp2LoFreq );
    ...
    adsr = new ADSR( 1.0, fadeTime, 0.0, 1.0, fadeTime*40 );
    ...
    bpFilter1 = new BandPass( bp1Freq+(bp1Freq/2), bp1BW, out.sampleRate() );
