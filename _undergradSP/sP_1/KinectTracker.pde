...
class KinectTracker {
  ...
  void tracker() {
   ...
   /*-------------------------------------FRAME DIFFERENCING FOR PRESENCE SUM-------------------- 
    if (isRecording == true) {
    arraycopy(kinectVideoImage.pixels, backgroundPixels);
    ...
    for (int i = 0; i < numPixels; i++) { // For each pixel in the video frame...
      ...
      color currColor = kinect.getVideoImage.pixels[i];
  ...
  void fromEnterToRecording() {
    ...
    isPlaying = false; //makes sure its not playing
  }
