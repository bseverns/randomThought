class KinectTracker {
  ...
  void track() {
    ...
    if (whereIsVP == NOTHING) {
      ...
      if (currentPos == NOTHING) {
        whereIsVP = NOTHING;
        exited(); //reset bg after receive OSC from camera
      }
  ...
  void lightsOn() {//starts all of the things that we need to happen when the viewer enters zone 3
    ...
    println("ON!!!!");
    println("SOUND!!!!");
    OscMessage onMsg = new OscMessage("click");//sound on
    onMsg.add(1); // add an int to the osc message 
    oscP5.send(onMsg, photoBooth);
    println("SENT");
    lights = true;
  }

  void lightsOff() {//does all of the "reseting" things that happen when the viewer leaves
    OscMessage offMsg = new OscMessage("/click");//sound off
    offMsg.add(0); /* add an int to the osc message */
    oscP5.send(offMsg, photoBooth);
    println("NO SOUND!!!!");
